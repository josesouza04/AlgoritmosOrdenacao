...
void buildheap(int *A, int n, sortperf_t * s) {
  for (int i = n / 2 - 1; i >= 0; i--) heapify(A, i, n - 1, s);
}
...
void partition3(int * A, int l, int r, int *i, int *j, sortperf_t *s) {
  int p = median(A[l], A[(l+r)/2], A[r]); // escolhe o pivô como a mediana de 3
  *i = l;
  *j = r;
  while (*i <= *j) {
    while (A[*i] < p) {
...
void partition(int * A, int l, int r, int *i, int *j, sortperf_t *s) {
  int p = A[(l+r)/2]; // escolhe o pivô como o elemento do meio
  *i = l;
  *j = r;
  while (*i <= *j) {
    while (A[*i] < p) {
...
void quickSortIns(int * A, int l, int r, sortperf_t *s) { 
  if (l < r) {
    if (r - l + 1 <= 10) { // usa insertion sort para partições pequenas
